...
def _startup_interrupted(timeout_ms):
    led = OnboardLED()
    print(
        "[main] press Ctrl-C in the next {} ms to skip auto-start".format(timeout_ms)
    )
    try:
        start = ticks_ms()
...
def _run_configured_module():
    ...
    print("[main] starting {}.{}()".format(module_name, function_name))
    module = __import__(module_name)
    getattr(module, function_name)()


if _startup_interrupted(config.BOOT_REPL_PAUSE_MS):
    print("[main] Ctrl-C received; auto-start skipped")
else:
